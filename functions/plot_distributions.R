                                ...
                                y_axis_name = NULL, base_size = 11, ...) 
{
  ...
  standard_plot <- ggplot2::ggplot(standard_plot_data, ggplot2::aes(x = value, 
    ...
    ggplot2::labs(x = x_axis_name, y = y_axis_name, fill = "Decision option",color = "Decision option") + 
    ggplot2::theme_bw(base_size = base_size)
    
  if (method == "smooth_simple_overlay") {
    return(standard_plot +
             geom_density(alpha = 0.1, linewidth = 0.5) + # line automatically added
             ggplot2::theme(legend.position = c(0.98, 0.98),  # Legend in top-right corner
                            legend.justification = c(1, 1)) +
             ggplot2::theme(...))
  }
  if (method == "hist_simple_overlay") {
    return(standard_plot + ggplot2::geom_histogram(color = NA, 
                                                   alpha = 0.05, bins = 150) + ggplot2::theme(...))
  }
  if (method == "boxplot") {
  ...
  if (method == "boxplot_density") {
    ...
    return(ggplot2::ggplot(standard_plot_data, ggplot2::aes(x = value, 
                                                            fill = name)) + ggplot2::geom_density(alpha = 0.05, 
